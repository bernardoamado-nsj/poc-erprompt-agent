...
REGRA CRÍTICA: COMPOSIÇÃO (itens/linhas) vs ENTIDADE TOP-LEVEL

Quando a especificação descreve "itens/linhas" pertencentes a um documento (ex.: PedidoVenda -> itens):
- Assuma COMPOSIÇÃO por padrão.
- NÃO crie uma entidade top-level separada para os itens (NÃO inclua em "entities").
- Modele os itens como schema interno do pai em json_schema.components.schemas e referencie via $ref em json_schema.properties.itens (array).

Só crie uma entidade separada para itens se o P.O. pedir explicitamente CRUD/endpoint/listagem independente para itens,
reuso fora do pai, ou filtros/pesquisa sobre itens como objeto independente.

Quando houver composição, o spec_markdown da entidade pai DEVE conter explicitamente:
- a propriedade "itens" (array), se houver minItems
- o nome do schema interno em components.schemas (ex.: PedidoVendaItem)
- os campos do item (produto, quantidade, valorUnitario, total, etc.)
- regra: itens não têm endpoint/CRUD próprio


REGRA DE LAYOUT PARA COLEÇÃO INTERNA (composição)
- Para exibir itens do registro selecionado, NÃO crie entity_refs separado.
- Use o dataRef da coleção interna no formato:
  "<alias>.selected/itens"
  Exemplo: alias "pedidos" => dataRef "pedidos.selected/itens"
- Em tela de detalhe do pai, o grid/lista de itens deve apontar para esse dataRef.
